[Voltar ao indice](../README.md)

### Exemplo de prompt (ReAct) — Consulta de Pedido
Caso de uso: quando existe uma inconsistencia entre o que aconteceu no sistema e o que o usuario enxerga, e e preciso investigar com consultas e acoes corretivas. Aqui, o fluxo verifica o pedido, identifica a falha e corrige o estado.

Entrada:
```code-block
O usuario informou que o pedido foi cobrado, mas aparece como pendente no sistema. Resolva usando ReAct.

Passo 1 - Pensamento: Verifique o status atual do pedido no banco de dados
Acao: Use a tool ficticia `DatabaseQueryTool` para consultar a tabela de pedidos pelo `orderId`
Observacao:
- orderId: 84521
- status: "PENDING"
- paymentStatus: "PAID"
- updatedAt: "2026-03-18T09:42:00Z"

Passo 2 - Pensamento: Se o pagamento foi confirmado, preciso verificar se houve falha na atualizacao do pedido
Acao: Use a tool ficticia `DatabaseQueryTool` para consultar a tabela de eventos de processamento desse pedido
Observacao:
- event: "ORDER_STATUS_UPDATE"
- result: "ERROR"
- message: "timeout ao atualizar status para CONFIRMED"

Passo 3 - Pensamento: O pedido foi pago, mas a atualizacao falhou; o proximo passo e corrigir o estado inconsistente
Acao: Use a tool ficticia `OrderUpdateTool` para atualizar o status do pedido para `CONFIRMED`
Observacao:
- updateStatus: "SUCCESS"

Resposta final:
- causa provavel: falha na atualizacao do status do pedido apos confirmacao do pagamento
- evidencias: pedido com `paymentStatus` igual a `PAID`, status ainda `PENDING` e evento com erro de timeout
- orientacao recomendada: informar ao usuario que o pedido foi corrigido e solicitar nova validacao no sistema

Agora processe o caso abaixo seguindo os mesmos 3 passos.
```

### Diagrama de Fluxo

```mermaid
flowchart TD
    A[Entrada: pedido cobrado, mas ainda pendente] --> B[Prompt: seguir ciclos de Pensamento, Acao e Observacao]
    B --> C[Pensamento 1: qual e o estado atual do pedido?]
    C --> D[Acao 1: consultar pedido com DatabaseQueryTool]
    D --> E[Observacao 1: status PENDING e pagamento PAID]
    E --> F[Pensamento 2: houve falha na atualizacao?]
    F --> G[Acao 2: consultar eventos do pedido]
    G --> H[Observacao 2: erro de timeout ao confirmar]
    H --> I[Pensamento 3: corrigir o estado inconsistente]
    I --> J[Acao 3: executar OrderUpdateTool]
    J --> K[Observacao 3: atualizacao concluida]
    K --> L[Saida final: pedido corrigido e usuario orientado]

    classDef input fill:#2563eb,stroke:#1e3a8a,color:#fff
    classDef prompt fill:#475569,stroke:#334155,color:#fff
    classDef technique fill:#d97706,stroke:#92400e,color:#fff
    classDef reasoning fill:#7c3aed,stroke:#5b21b6,color:#fff
    classDef action fill:#0f766e,stroke:#115e59,color:#fff
    classDef observation fill:#0891b2,stroke:#0e7490,color:#fff
    classDef decision fill:#dc2626,stroke:#991b1b,color:#fff
    classDef output fill:#16a34a,stroke:#166534,color:#fff

    class A input
    class B prompt
    class C,F,I reasoning
    class D,G,J action
    class E,H,K observation
    class L output
```

> **Caracteristica:** ReAct aplicado a inconsistencia de dados. O ciclo Pensamento-Acao-Observacao investiga camada por camada ate resolver o estado inconsistente.
